...
```powershell
irm https://raw.githubusercontent.com/Patryk-beep/powershell-script-hub/v1.7.0.0/install-hub.ps1 | iex
```
...
If GitHub rate-limits the request (rare), pass `-Version v1.7.0.0` to skip the
`latest` lookup:

```powershell
$args = @{ Version = 'v1.7.0.0' }
& ([scriptblock]::Create((irm https://raw.githubusercontent.com/Patryk-beep/powershell-script-hub/v1.7.0.0/install-hub.ps1))) @args
```
...
| `-Version v1.7.0.0` | Pin a specific release tag (default). |
| `-VerifyHash <sha256>` | Override the embedded hash check (e.g. when piping a custom build). |

## Update

```powershell
irm https://raw.githubusercontent.com/Patryk-beep/powershell-script-hub/v1.7.0.0/install-hub.ps1 | iex -Update
```
...
itself (planned for v1.2; for now, run with `-Version v1.7.0.0` and let it
...
- The installer URL is **tag-pinned** (`/v1.7.0.0/`), not `/main/`. A compromised
